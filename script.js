...
function playGame() {
    ...
    guessCheck(numberGuess, randomNo);
...
function guessCheck(numberGuess, randomNo) {
...
function getRandomNumber() {
    // *CODE GOES BELOW HERE *
    console.log("Inside get Random Number" + Math.random());
...
function displayHistory() {
    ...
    let list = "<ul class='list-group history '>";
    // *CODE GOES BELOW HERE *
    while (index >= 0) {
        list += '<li class="list-group-item history">';
        list += 'You guessed ' + guesses[index];
        list += '</li>';
        index--;
    }
    list += '</ul>'
...
function getDialog(dialogType, text) {
    ...
    switch (dialogType) {
        case "warning":
            dialog = "<div class='alert alert-warning result-border' role='alert'>"
            break;
        case "won":
            dialog = "<div class='alert alert-success success-div' role='alert'>"
            break;
    }
    dialog += text;
    dialog += "</div>"
...
function showYouWon() {
    ...
    $(this).css({
        'background-image': "url(D://a6392f519c8e01fa9c44fe25b171038e.gif)",
    });
}
